...
def test_report_holds_experiment_sequence_identity_and_metadata():
    se = _make_experiment()
    report = Report(
        experiment=se,
        sequence_identity={"s1:ASV0": 100.0, "s1:ASV1": 98.0, "s1:ASV2": 90.0},
        metadata={"note": "test"},
    )
    assert report.experiment is se
    assert report.sequence_identity == {"s1:ASV0": 100.0, "s1:ASV1": 98.0, "s1:ASV2": 90.0}
...
def test_sequence_identity_pct_none_when_empty():
    report = Report(experiment=_make_experiment())
    assert report.sequence_identity_pct is None


def test_sequence_identity_pct_is_max_of_per_otu_identity():
    # Reports render at species level; identity collapses per-OTU with max().
    # TODO: revisit — placeholder aggregation (see Report.sequence_identity_pct).
    se = _make_experiment()
    report = Report(
        experiment=se,
        sequence_identity={"s1:ASV0": 100.0, "s1:ASV1": 90.0, "s1:ASV2": 50.0},
    )
    assert report.sequence_identity_pct == pytest.approx(100.0)


def test_sequence_identity_pct_max_without_abundance_assay():
    ...
    report = Report(experiment=se, sequence_identity={"s1:ASV0": 100.0, "s1:ASV1": 80.0})
    assert report.sequence_identity_pct == pytest.approx(100.0)
...
def test_with_sequence_identity_returns_new_object_without_mutation():
    report = Report(experiment=_make_experiment())
    updated = report.with_sequence_identity({"s1:ASV0": 99.0})
    assert report.sequence_identity == {}  # original untouched
    assert updated.sequence_identity == {"s1:ASV0": 99.0}
